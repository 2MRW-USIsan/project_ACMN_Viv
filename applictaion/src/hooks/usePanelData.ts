  ...
  return useMemo(() => {
          ...
          data: child.data.map((itemData) => ({
            ...
            complexData:
              child.values.type === "Complex"
                ? (itemData.complexData ?? []).map((complexItemData) => ({
                    ...
                    onChangeForm: (label: string, value: string) =>
                      actions.changeItemForm(
                        ...
                        "orders",
                        item.id,
                        `child:${child.id}:item:${itemData.id}:complexItem:${complexItemData.id}:${label}`,
                        value,
                      ),
                    onDelete: () =>
                      actions.deleteOrdersComplexItemDataPanel(
                        panelId,
                        item.id,
                        child.id,
                        itemData.id,
            ...
            onDelete: () =>
              actions.deleteOrdersItemDataPanel(
                panelId,
                item.id,
                child.id,
                itemData.id,
      ...
      items.map((item) => ({
        ...
        data: item.data.map((child) => ({
          ...
          data: child.data.map((listItem) => ({
            ...
            onDelete: () =>
              actions.deleteSelectListItemPanel(
                panelId,
                item.id,
                child.id,
                listItem.id,
    ...
    const mapSwitchItems = (
      ...
      items.map((item) => ({
        ...
        onToggleRandomize: () =>
          actions.toggleSwitchRandomize(panelId, item.id),
