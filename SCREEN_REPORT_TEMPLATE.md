...
このドキュメントは **AI が画面実装を完了した際に出力する報告書・成果物仕様書** のフォーマットです。  
実装依頼に対してAIが生成する成果物として、実装済みの設計・構成を記録します。  
テンプレートの記入例として [`/editor` 画面の成果物仕様書](#editor-画面成果物仕様書--bloc-リストエディタ) を末尾に添付しています。

> 画面実装を**依頼する側**が記述する入力ドキュメントは [`SCREEN_REQUEST_TEMPLATE.md`](./SCREEN_REQUEST_TEMPLATE.md) を参照してください。
